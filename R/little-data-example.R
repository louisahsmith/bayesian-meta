...
for (i in seq_along(outcomes)) {
  res_10[[i]] <- metaanalysis_grid(outcomes[i], all_dat = all_dat, mu_prior_sd = 10,
                                   iter = 2000, refresh = 100)
}
for (i in seq_along(outcomes)) {
  res_1[[i]] <- metaanalysis_grid(outcomes[i], all_dat = all_dat, mu_prior_sd = 1,
                                  iter = 5000)
}
for (i in seq_along(outcomes)) {
  res_01[[i]] <- metaanalysis_grid(outcomes[i], all_dat = all_dat, mu_prior_sd = 0.1,
                                   iter = 5000)
}

summary_stats <- map_dfr(list(res_10, res_1, res_01), 
                         ~map_dfr(.x, pluck, "summary_stats"), .id = "prior") %>% 
  mutate(prior = factor(prior, labels = c("10", "1", "0.1")))

all_draws <- map_dfr(list(res_10, res_1, res_01),
                     ~map_dfr(.x, pluck, "draws"), .id = "prior") %>% 
  mutate(prior = factor(prior, labels = c("10", "1", "0.1")))

all_plots <- map(list(res_10, res_1, res_01), 
                 ~map(.x, pluck, "plot"))
...
all_dat %>% 
  ...
  select(method, ends_with("Id"), contains("estimate")) %>% 
  gt::gt()


summary_stats %>% 
  filter(.variable == "mu") %>% 
  select(outcomeId, median, q5, q95) %>% 
  mutate(across(c(median, q5, q95), exp)) %>% 
  mutate(`Overall estimate` =  make_ci(median, q5, q95)) %>% 
  select(outcomeId, `Overall estimate`) %>% 
  gt::gt()
