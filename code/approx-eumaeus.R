...
sim_function <- function(i, positive_control_outcomes, all_dat, iter = 3000, chains = 1, regular = TRUE, ...) {
  ...
  dat <- list(J = nrow(NCs_all), # number of negative controls overall
              ...
              grid_id = matrix(for_grid$rowid, ncol = 2, byrow = TRUE), # start and end indices of the log-likehood vectors
  ...
  mod <- stan(here::here("stan", "NCs-multiple-sites-priors-real-effect-likelihood-irregular.stan"),
              data = c(dat, priors), iter = iter, chains = chains,
              control = list(adapt_delta = 0.9999, max_treedepth = 15))
  }
...
res <- future_map(1:nrow(positive_control_outcomes), sim_function, 
                    positive_control_outcomes, all_dat, iter = 2000, chains = 3, regular = TRUE)
plan(sequential)
